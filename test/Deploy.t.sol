...
contract DeployTest is Base {
    ...
    function test_index_and_recipient_vary_address() public view {
        require(
            factory.computeAddress(_r(), 0, false) != factory.computeAddress(_r(), 1, false), "index must vary address"
        );
        require(
            factory.computeAddress(_r(), 0, false) != factory.computeAddress(_r2(), 0, false),
            "recipient must vary address"
        );
    }
    ...
    function test_no_hijack() public {
        address victimAddr = factory.computeAddress(_r(), 1, false);
        usdc.mint(victimAddr, 100e6);
        address attackerAddr = factory.deploy(_r2(), 1, false);
    ...
    function test_deploy_idempotent() public {
        address a = factory.deploy(_r(), 0, false);
        address b = factory.deploy(_r(), 0, false);
        require(a == b && a.code.length > 0 && factory.isDeployed(_r(), 0, false), "deploy must be idempotent");
    }
    ...
    function test_deploy_accepts_arbitrary_recipient() public {
        ...
        address predicted = factory.computeAddress(weird, 0, false);
